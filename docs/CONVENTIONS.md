...
4. **Local-first, no cloud dependencies.** Foundry runs entirely on the local workstation. Do not add cloud APIs, SaaS dependencies, or remote database connections. All data stays local.
...
- Use `ConfigureAwait(false)` in library code (`Foundry.Core`). Omit in broker code (`Foundry.Broker`).
...
Foundry/
├── src/
│   ├── Foundry.Core/         # Class library (net10.0, no Windows dependency)
│   │   ├── Models/             # All data models (Foundry.Models namespace)
│   │   └── Services/           # All services (Foundry.Services namespace)
│   └── Foundry.Broker/       # ASP.NET Core minimal API (Foundry.Broker namespace)
...
├── scripts/                  # Python ML scripts + PowerShell automation
│   ├── ml/                     # scikit-learn / PyTorch / TensorFlow scripts
│   ├── rag/                    # RAG pipeline scripts
│   ├── scoring/                # Scoring and replay scripts
│   ├── automation/             # PowerShell automation (PR review, etc.)
│   └── commands/               # Misc command scripts
...
|-------------|----------|-----|
| New model used everywhere | `src/Foundry.Core/Models/` | Single source of truth for all data models |
...
| Python ML/scoring scripts | `scripts/ml/` or `scripts/scoring/` | Invoked via ProcessRunner |
...
- Scripts live in `scripts/ml/`, `scripts/scoring/`, `scripts/rag/`, `scripts/automation/`, or `scripts/commands/`.
...
- JSON export available for cross-machine sharing via Tailscale SMB (`FOUNDRY_STATE_ROOT`), not Dropbox.
